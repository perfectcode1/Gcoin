...
$storeName = "Versace Shop";
session_start();
include('configuration.php');
include('gmessage.php');
include('gtools.php');
include('gwire.php');
$items = array(array("name"=>"versace Shoe","desc"=>"The walking machine","amount"=>3),array("name"=>"Rolex Watch","desc"=>"Market's best wrist watch","amount"=>5),array("name"=>"Polo Tshirt","desc"=>"Golden royal colored polo tshirt","amount"=>7),array("name"=>"Bracelet","desc"=>"The best of bracelets","amount"=>10));
$step = 1;
if(isset($_POST['step1'])){
	if(!isset($_POST['retry'])){
		...
		$_SESSION['shop2']['sum'] = $sum;
		$_SESSION['shop2']['purchase'] = $purchase;
	}
...
if(isset($_POST['step2'])){
	...
	$_SESSION['shop2']['address'] = $address;
	$_SESSION['shop2']['order id'] = time();
	...
	$params['order id'] = $_SESSION['shop2']['order id'];
	$params['time'] = time();	
...
<?php }?>
<?php if($step==2){?>
<p> You have chosen to purchase the following items: <?php echo join(", ", $_SESSION['shop2']['purchase']);?>; valued at <?php echo "$sum $currency";?></p>
